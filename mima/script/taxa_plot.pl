...
ggplot(otable, aes(x=vars, y = values, fill =rows)) +
  geom_bar(position = "stack", stat = "identity") +  xlab("Samples") + ylab("Percentage") + 
    theme(panel.background=element_rect(fill="white",color="black"),panel.grid.major=element_line(size=0,color="white"),panel.grid.minor=element_line(size=0,color="white"),legend.key = element_rect(fill = "white"), axis.text.x = element_text(angle = 90)) 
dev.off()
RSP
print RS $text; 
